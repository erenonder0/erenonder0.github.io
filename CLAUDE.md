...
- **Experience** (`layouts/partials/experience.html`): full-width single-column cards (Option C), one per entry, no right-hand column. Each card: logo (44×44 rounded tile; image with initials fallback) + date/role/company/location, a "Current" badge with pulsing green dot for the active position (detected via `duration` containing "now"), then a collapsible block holding the description and skill-tag pills (from each entry's `tags` front matter). The logo tile's treatment comes from `logoStyle` in the front matter: `fill` (default) lets a mark with its own background cover the tile, `plate` puts a dark-ink transparent logo on white. Certification logos in `data/certifications.yaml` use the same idea via `logo_style`, with an extra `badge` value for round seals. Cards are **collapsed by default** with a blue "Show more ▾"/"Show less ▴" toggle (`.exp-card__toggle`); clicking the toggle flips `.is-expanded` on the card (`.exp-card__collapse` animates max-height + opacity, 300ms; multiple cards may stay open). The active card renders with `.is-expanded` on page load. Toggle logic lives in `initExpanders()` in `assets/js/portfolio-animations.js`. Active card gets a 3px blue left border and a faint blue tint (`.exp-card--active`). Entries come from `content/experience/*.md`.
